...
fn main() {
    ...
    // the child threads.
    let (ts, rs) = channel::unbounded();
    let rs2 = rs.clone();

    for i in 1..50 {
        println!("{}", i);
        ts.send(i.to_string()).unwrap();
    }

    let handle_c1 = thread::spawn(move || rs.recv().unwrap());
    let handle_c2 = thread::spawn(move || rs2.recv().unwrap());

    println!("C1 receives {}", handle_c1.join().unwrap());
    println!("C2 receives {}", handle_c2.join().unwrap());
